...
model = dict(
    ...
    lane_head=dict(
        ...
        llm_cfg=dict(
            ...
            set_lora=False,  # ❌ Freeze LLM - from HERMES stage2-1
            is_pretraining=True,  # Pretraining mode - from HERMES stage2-1
            attention_type='flash_attention_2',  # Attention type
            ...
            chat_config=dict(  # Chat generation config - from HERMES stage2-1
                num_beams=1,
                max_new_tokens=8192,
                min_new_tokens=1,
                do_sample=False,
...
data = dict(
    ...
    train=dict(
        ...
        train_conv_path='./data/train_conv_rdp',  # Path to LLM conversation data
...
# HERMES stage2-1: Gradient accumulation with FP32 master weights
# - cumulative_iters=4: Accumulate gradients over 4 iterations before updating
# - loss_scale='dynamic': Dynamic loss scaling for FP16 training stability
# - Even though LLM uses BF16, optimizer maintains FP32 master weights
...
evaluation = dict(interval=num_epochs*num_iters_per_epoch+100)
runner = dict(
...
load_from = './work_dirs/stream/stage1/latest.pth'
#load_from = None
resume_from = None
workflow = [('train', 1)]
...
# HERMES stage2-1: Freeze LLM parameters
fix_params = True
tuning_layers = []