...
class CustomFormatBundle3DLane(DefaultFormatBundle3D):
    ...
    def __call__(self, results):
        ...
        if 'gt_future_waypoint' in results:
            results['gt_future_waypoint'] = DC(
                to_tensor(results['gt_future_waypoint']))
        if 'gt_ego_fut_cmd' in results:
            results['gt_ego_fut_cmd'] = DC(
                to_tensor(results['gt_ego_fut_cmd']))
        # Add LLM conversation data to formatted results
        if 'llm_input_ids' in results and results['llm_input_ids'] is not None:
            results['llm_input_ids'] = DC(to_tensor(results['llm_input_ids']))
        if 'llm_attention_mask' in results and results['llm_attention_mask'] is not None:
            results['llm_attention_mask'] = DC(to_tensor(results['llm_attention_mask']))
        if 'llm_labels' in results and results['llm_labels'] is not None:
            results['llm_labels'] = DC(to_tensor(results['llm_labels']))
