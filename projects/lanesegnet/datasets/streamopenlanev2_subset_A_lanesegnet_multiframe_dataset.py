...
from projects.lanesegnet.datasets.visualize.renderer import Renderer
import pickle
@DATASETS.register_module()
class StreamOpenLaneV2_subset_A_LaneSegNet_Multiframe_Dataset(BaseCustom3DDataset): ###继承的是Custom3DDataset
    ...
    def get_data_info(self, index):
        ...
        # input_dict['sample_idx'] = info['sample_idx']  # REMOVED: don't override with array index
    ...
    def prepare_train_data(self, index):
        ...
        if self.filter_empty_te and \
            ...
            return None
        ###multiframe
        seq_info = self.idx2scene[index]
        example['seq_info'] = DC(seq_info, cpu_only=True)

        if self.multi_frame: # used when sampling multi-frame training data
            scene_name = input_dict['scene_token']
            ...
            example['img_metas'].data['local_idx'] = local_idx_curr
            ...
            for idx, global_idx_prev in enumerate(global_indices_prev):
                input_dict_prev = self.get_data_info(global_idx_prev)
            ...
            example['all_prev_data'] = all_prev_data
            example['all_local2global_info'] = all_local2global_info
    ...
    def evaluate(self, results, logger=None, show=False, out_dir=None, **kwargs):
        ...
        gt_dict = mmcv.load("./results/gt_results.pkl", file_format="pkl")
        print('len of results', len(results))
        print('len of gt_dict', len(gt_dict))
        # assert len(results) == len(gt_dict)
