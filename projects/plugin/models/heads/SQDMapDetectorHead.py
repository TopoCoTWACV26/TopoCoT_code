...
class SQDMapDetectorHead(nn.Module):
    ...
    def denoise(self, gt_vecs_list, gt_labels_list, device):
        ...
        if self.dn_cfg is not None:
            ...
            pe_pts = self.dn_pts(gen_sineembed_for_position(denoise_refers.flatten(1, 2), self.embed_dims//4)) ##用sine PE
            pe_pts = pe_pts.reshape(pe_pts.size(0), -1, self.num_points, pe_pts.size(2)).flatten(2)
            dn_query_pos = self.instance_convert(pe_pts) ##再encode一下PE
            query = self.query_convert(torch.cat((dn_query_content, dn_query_pos), -1)) ###构建noise query
    ...
    def forward_train(self, bev_features, img_metas, gts):
        ...
        bev_features = self._prepare_context(bev_features) ###给bev加PE
        ...
        query_embedding = self.query_embedding.weight[None, ...].repeat(bs, 1, 1) # [B, num_q, embed_dims] ##learnable queries 取了四次 bs=4
        ...
        # stream dn target获取
        last_gt = self.stream_dn_target_memory.get(img_metas) #{'gt': [None, None, None, None], 'label_list': [None, None, None, None], 'is_first_frame': [True, True, True, True], 'id_list': [None, None, None, None]}
        ...
        for i, (queries) in enumerate(inter_queries):
            reg_points = inter_references[i][:, dn_num:] # (bs, num_q, num_points, 2) ###不使用DN query
            bs = reg_points.shape[0]
            reg_points = reg_points.view(bs, -1, 2*self.num_points) # (bs, num_q, 2*num_points)

            scores = self.cls_branches[i](queries[:, dn_num:]) # (bs, num_q, num_classes) ###不使用DN query
        ...
        if dn_num != 0:
            outputs_dn = []
            for i, (queries) in enumerate(inter_queries):
                reg_points = inter_references[i][:, :dn_num] # (bs, num_q, num_points, 2) ##取出DN query
                bs = reg_points.shape[0]
                reg_points = reg_points.view(bs, -1, 2*self.num_points) # (bs, num_q, 2*num_points)
