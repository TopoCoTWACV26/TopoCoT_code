...
def merge_divider(vecs=None, thickness=1):
    ...
    for vec in vecs:
        ...
        # If IOU is not 0, merge the new line with the line in the merged_lines
            ...
            merged_lines = iou_merge_divider(merged_lines, vec, thickness=5)
...
def get_consecutive_vectors_with_opt( prev_vectors=None,prev2curr_matrix=None,origin=None,roi_size=None, denormalize=False, clip=False):
    ...
    prev2curr_vectors = []
    # prev2curr_labels = []
    for ids, vecs in enumerate(prev_vectors):
        ...
        normed_vecs = transformed_vecs[..., :3] 
        
        prev2curr_vectors.append(normed_vecs.reshape(30,3))
        # prev2curr_labels.append(labels[ids])
...
def plot_fig_merged_per_frame(num_frames, car_trajectory, x_min, x_max, y_min, y_max, pred_save_folder, id_prev2curr_pred_vectors, id_prev2curr_pred_frame, id_prev2curr_pred_left_types, id_prev2curr_pred_right_types, args):
    os.makedirs(pred_save_folder, exist_ok=True)
  
    # key the current status of the instance, add into the dict when it first appears
    instance_bank = dict()
    instance_bank_left = dict()
    instance_bank_right = dict()
    instance_bank_left_type = dict()
    instance_bank_right_type = dict()
    ...
    for frame_timestep in range(num_frames):
        ...
        for (vec_tag, vec_all_frames), (_ , left_type_all_frames), (_, right_type_all_frames) in zip(id_prev2curr_pred_vectors.items(), id_prev2curr_pred_left_types.items(), id_prev2curr_pred_right_types.items()):
            ...
            if need_merge:
                ...
                all_vec = vec_all_frames[vec_index_in_instance]
                curr_left_type = left_type_all_frames[vec_index_in_instance]
                curr_right_type = right_type_all_frames[vec_index_in_instance]

                curr_vec = all_vec[:10,:]
                curr_vec_left = all_vec[10:20,:]
                curr_vec_right = all_vec[20:,:]

                curr_vec_polyline = LineString(curr_vec)
                curr_vec_polyline_left = LineString(curr_vec_left)
                curr_vec_polyline_right = LineString(curr_vec_right)
                if vec_tag not in instance_bank: # if the instance first appears
                    polylines = [curr_vec_polyline,]
                    polylines_left = [curr_vec_polyline_left,]
                    polylines_right = [curr_vec_polyline_right,]
                    type_left = [curr_left_type, ]
                    type_right = [curr_right_type, ]
                    ...
                    polylines_left = instance_bank_left[vec_tag] + [curr_vec_polyline_left,]
                    polylines_right = instance_bank_right[vec_tag] + [curr_vec_polyline_right,]
                    type_left = instance_bank_left_type[vec_tag] + [curr_left_type,]
                    type_right = instance_bank_right_type[vec_tag] + [curr_right_type,]

            else: # if the instance has not appeared in this frame
                polylines = instance_bank[vec_tag]
                polylines_left = instance_bank_left[vec_tag] 
                polylines_right = instance_bank_right[vec_tag]
                type_left = instance_bank_left_type[vec_tag]
                type_right = instance_bank_right_type[vec_tag]

            if label == 0: # centerline
                color = 'orange'
            elif label == 1: # pc
                color = 'b'
            ...
            if need_merge:
                ...
                polylines_vecs_left = [np.array(one_line.coords) for one_line in polylines_left]
                polylines_vecs_left = merge_divider(polylines_vecs_left)

                polylines_vecs_right = [np.array(one_line.coords) for one_line in polylines_right]
                polylines_vecs_right = merge_divider(polylines_vecs_right)
            else:  # if no new instance, use the previous merged polyline to plot
                polylines_vecs = [np.array(line.coords) for line in polylines]
                polylines_vecs_left = [np.array(line.coords) for line in polylines_left]
                polylines_vecs_right = [np.array(line.coords) for line in polylines_right]
            ...
            for one_line, type_left_single in zip(polylines_vecs_left,type_left):
                ...
                config = LINE_PARAM[type_left_single]
                if label == 0:
                    ax.plot(x, y, linewidth=20, **config)
                # ax.plot(x, y, "o", color=color, markersize=50)

            # update instance bank for line
            updated_polylines = [LineString(vec) for vec in polylines_vecs_left]
            instance_bank_left[vec_tag] = updated_polylines
            instance_bank_left_type[vec_tag] = [left for left in type_left]

            for one_line, type_right_single in zip(polylines_vecs_right, type_right):
                one_line = np.array(LineString(one_line).simplify(args.simplify*2).coords)
                pts = one_line[:, :2]
                x = np.array([pt[0] for pt in pts])
                y = np.array([pt[1] for pt in pts])

                config = LINE_PARAM[type_right_single]
                if label == 0:
                    ax.plot(x, y, linewidth=20, **config)
                # ax.plot(x, y, "o", color=color, markersize=50)

            # update instance bank for line
            updated_polylines = [LineString(vec) for vec in polylines_vecs_right]
            instance_bank_right[vec_tag] = updated_polylines
            instance_bank_right_type[vec_tag] = [right for right in type_right]
...
def plot_fig_merged(car_trajectory, x_min, x_max, y_min, y_max, pred_save_path, id_prev2curr_pred_vectors, id_prev2curr_pred_left_types, id_prev2curr_pred_right_types, args):
    ...
    for (tag, vecs), (_ , left_type_all_frames), (_, right_type_all_frames)  in zip(id_prev2curr_pred_vectors.items(), id_prev2curr_pred_left_types.items(), id_prev2curr_pred_right_types.items()):
        ...
        polylines_right = []
        for vec in vecs:
            polylines.append(LineString(vec[:10,:]))
            polylines_left.append(LineString(vec[10:20,:]))
            polylines_right.append(LineString(vec[20:,:]))
        ...
        if label==0:
            polylines_vecs = [np.array(one_line.coords) for one_line in polylines_left]
            polylines_vecs = merge_divider(polylines_vecs)
            for one_line, left_type in zip(polylines_left, left_type_all_frames):
                one_line = np.array(LineString(one_line).simplify(args.simplify).coords)
                pts = one_line[:, :2]
                x = np.array([pt[0] for pt in pts])
                y = np.array([pt[1] for pt in pts])
                config = LINE_PARAM[left_type]
                ax.plot(x, y, linewidth=20, **config)

            polylines_vecs = [np.array(one_line.coords) for one_line in polylines_right]
            polylines_vecs = merge_divider(polylines_vecs)
            for one_line, right_type in zip(polylines_right, right_type_all_frames):
                one_line = np.array(LineString(one_line).simplify(args.simplify).coords)
                pts = one_line[:, :2]
                x = np.array([pt[0] for pt in pts])
                y = np.array([pt[1] for pt in pts])
                config = LINE_PARAM[right_type]
                ax.plot(x, y, linewidth=20, **config)
...
def plot_fig_unmerged_per_frame(num_frames, car_trajectory, x_min, x_max, y_min, y_max, pred_save_folder, id_prev2curr_pred_vectors, id_prev2curr_pred_frame, id_prev2curr_pred_left_types, id_prev2curr_pred_right_types, args):
    ...
    for frame_timestep in range(num_frames):
        ...
        for (vec_tag, vec_all_frames), (_ , left_type_all_frames), (_, right_type_all_frames) in zip(id_prev2curr_pred_vectors.items(), id_prev2curr_pred_left_types.items(), id_prev2curr_pred_right_types.items()):
            ...
            if label == 0:
                polyline = LineString(curr_vec_left)
                vector = np.array(polyline.coords)
                pts = vector[:, :2]
                x = np.array([pt[0] for pt in pts])
                y = np.array([pt[1] for pt in pts])
                config = LINE_PARAM[curr_left_type]
                ax.plot(x, y, linewidth=20, **config)

                polyline = LineString(curr_vec_right)
                vector = np.array(polyline.coords)
                pts = vector[:, :2]
                x = np.array([pt[0] for pt in pts])
                y = np.array([pt[1] for pt in pts])
                config = LINE_PARAM[curr_right_type]
                ax.plot(x, y, linewidth=20, **config)
...
def plot_fig_unmerged(car_trajectory, x_min, x_max, y_min, y_max, pred_save_path, id_prev2curr_pred_vectors, id_prev2curr_pred_left_types, id_prev2curr_pred_right_types, args):
...
def vis_pred_data(scene_name="", pred_results=None, origin=None, roi_size=None, args=None):
    ...
    id_prev2curr_pred_left_types = defaultdict(list)
    id_prev2curr_pred_right_types = defaultdict(list)
    ...
    for index in index_list:
        ...
        labels = pred_results[index]["labels"]
        left_type = pred_results[index]["left_types"]
        right_type = pred_results[index]["right_types"]

        curr_vectors = vectors

        # get the transformation matrix of the last frame
        prev_e2g_trans =  torch.tensor(pred_results[index]['meta']['lidar2global_translation'], dtype=torch.float64)
        prev_e2g_rot = torch.tensor(pred_results[index]['meta']['lidar2global_rotation'], dtype=torch.float64)
        curr_e2g_trans  = torch.tensor(pred_results[last_index]['meta']['lidar2global_translation'], dtype=torch.float64)
        curr_e2g_rot = torch.tensor(pred_results[last_index]['meta']['lidar2global_rotation'], dtype=torch.float64)
        ...
        prev2curr_pred_vectors = get_prev2curr_vectors(curr_vectors[:,:,:2], prev2curr_matrix,origin[:2],roi_size[:2],False,False)
        prev2curr_pred_vectors = prev2curr_pred_vectors * roi_size[:2] + origin[:2]
        
        rotation_degrees = np.degrees(np.arctan2(prev2curr_matrix[:3, :3][1, 0], prev2curr_matrix[:3, :3][0, 0]))
        car_center = get_prev2curr_vectors(np.array((0,0)).reshape(1,1,2), prev2curr_matrix,origin[:2],roi_size[:2],False,False)* roi_size[:2] + origin[:2]
        ...
        for i, (label, vec_glb_idx) in enumerate(zip(pred_results[index]['labels'], pred_results[index]['global_ids'])):
            ...
            id_prev2curr_pred_left_types[dict_key].append(left_type[i])
            id_prev2curr_pred_right_types[dict_key].append(right_type[i])
    ...
    id_prev2curr_pred_left_types = {key: id_prev2curr_pred_left_types[key] for key in sorted(id_prev2curr_pred_left_types)}
    id_prev2curr_pred_right_types = {key: id_prev2curr_pred_right_types[key] for key in sorted(id_prev2curr_pred_right_types)}
    ...
    if args.per_frame_result:
        ...
        plot_fig_merged_per_frame(num_frames, car_trajectory, x_min, x_max, y_min, y_max, pred_save_folder, id_prev2curr_pred_vectors, id_prev2curr_pred_frame, id_prev2curr_pred_left_types, id_prev2curr_pred_right_types, args)
        ...
        plot_fig_unmerged_per_frame(num_frames, car_trajectory, x_min, x_max, y_min, y_max, pred_save_folder, id_prev2curr_pred_vectors, id_prev2curr_pred_frame, id_prev2curr_pred_left_types, id_prev2curr_pred_right_types, args)
    pred_save_path = os.path.join(scene_dir, f'pred_unmerged.png')
    plot_fig_unmerged(car_trajectory, x_min, x_max, y_min, y_max, pred_save_path, id_prev2curr_pred_vectors, id_prev2curr_pred_left_types, id_prev2curr_pred_right_types, args)
    pred_save_path = os.path.join(scene_dir, f'pred_merged.png')
    plot_fig_merged(car_trajectory, x_min, x_max, y_min, y_max, pred_save_path, id_prev2curr_pred_vectors, id_prev2curr_pred_left_types, id_prev2curr_pred_right_types, args)
...
def form_lane_data(lanes, labels):
    result = {}
    num = len(labels)
    lanes = lanes.reshape(num, 30, 3)
    lanes = lanes[:,:,:2]
    # 遍历 label 中的每个唯一值
    for unique_label in np.unique(labels):
        # 获取对应标签的索引

        # indices = (labels == unique_label).nonzero(as_tuple=True)[0]
        indices = [index for index, value in enumerate(labels) if value == unique_label]

        # 根据索引从 pred 中提取切片
        slices = lanes[indices]
        # 将切片保存到字典
        result[int(unique_label.item())] = [slices[i] for i in range(slices.shape[0])]
    return result

def form_lane_type(lanes_type, labels):
    result = {}
    num = len(labels)
    # 遍历 label 中的每个唯一值
    for unique_label in np.unique(labels):
        ...
        # 根据索引从 pred 中提取切片
        # slices = lanes_type[indices]
        slices = [lanes_type[i] for i in indices]
        # 将切片保存到字典
        result[int(unique_label.item())] = [slices[i] for i in range(len(slices))]
    return result
...
def vis_gt_data(scene_name, args, dataset, gt_data, origin, roi_size):
    ...
    for idx in range(scene_len):
        ...
        right_type = []
        labels= []
        for ids, vecs in enumerate(gt_info_list[idx]['gt_lanes_3d'].data.numpy()) :

            label = gt_info_list[idx]['gt_lane_labels_3d'].data.numpy()[ids]
            labels.append(label)
            left_type_single = gt_info_list[idx]['gt_lane_left_type'].data.numpy()[ids]
            left_type.append(left_type_single)
            right_type_single = gt_info_list[idx]['gt_lane_right_type'].data.numpy()[ids]
        ...
        prev_e2g_trans = torch.tensor(gt_info_list[idx]['img_metas'].data[0]['lidar2global_translation'], dtype=torch.float64)
        prev_e2g_rot = torch.tensor(gt_info_list[idx]['img_metas'].data[0]['lidar2global_rotation'], dtype=torch.float64)
        curr_e2g_trans  = torch.tensor(gt_info_list[scene_len-1]['img_metas'].data[0]['lidar2global_translation'], dtype=torch.float64)
        curr_e2g_rot = torch.tensor(gt_info_list[scene_len-1]['img_metas'].data[0]['lidar2global_rotation'], dtype=torch.float64)
        prev_e2g_matrix = torch.eye(4, dtype=torch.float64)
        prev_e2g_matrix[:3, :3] = prev_e2g_rot
        prev_e2g_matrix[:3, 3] = prev_e2g_trans
        ...
        prev2curr_pred_vectors = np.stack(prev2curr_pred_vectors, axis=0)
        
        prev2curr_pred_vectors = form_lane_data(prev2curr_pred_vectors, labels) 
        prev2curr_pred_left_types = form_lane_type(left_type, labels)
        prev2curr_pred_right_types = form_lane_type(right_type, labels)
        ...
        for label, id_info in ids_info[idx].items():
            ...
            for vec_local_idx, vec_glb_idx in id_info.items():
                ...
                id_prev2curr_pred_left_types[dict_key].append(prev2curr_pred_left_types[label][vec_local_idx])
                id_prev2curr_pred_right_types[dict_key].append(prev2curr_pred_right_types[label][vec_local_idx])
    ...
    if args.per_frame_result:
        ...
        plot_fig_merged_per_frame(len(gt_info_list), car_trajectory, x_min, x_max, y_min, y_max, pred_save_folder, id_prev2curr_pred_vectors, id_prev2curr_pred_frame, id_prev2curr_pred_left_types, id_prev2curr_pred_right_types, args)
        
        pred_save_folder = os.path.join(scene_dir, f'gt_unmerged_per_frame')
        plot_fig_unmerged_per_frame(len(gt_info_list), car_trajectory, x_min, x_max, y_min, y_max, pred_save_folder, id_prev2curr_pred_vectors, id_prev2curr_pred_frame, id_prev2curr_pred_left_types, id_prev2curr_pred_right_types, args)
